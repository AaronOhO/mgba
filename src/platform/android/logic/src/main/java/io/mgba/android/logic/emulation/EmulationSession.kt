...
class EmulationSession(
    ...
    private data class LoadedRom(
        ...
        fun statePathFor(slot: Int): String {
            if (slot == QUICK_STATE_SLOT) return statePath
            val quickState = File(statePath)
            return File(quickState.parentFile, "${quickState.nameWithoutExtension}.ss$slot").path
        }
    }
    ...
    fun quickSave(slot: Int = QUICK_STATE_SLOT) {
        require(slot in QUICK_STATE_SLOT..MAX_STATE_SLOT) { "Invalid save-state slot: $slot" }
        executor.execute {
            val path = loadedRom?.statePathFor(slot) ?: return@execute
            val message = if (core.saveStateFile(path, settings.saveStateFlags())) {
                EmulationMessage.SAVE_STATE_SAVED
            } else {
                EmulationMessage.SAVE_STATE_FAILED
            }
            listener?.onMessage(message)
        }
    }

    fun quickLoad(slot: Int = QUICK_STATE_SLOT) {
        require(slot in QUICK_STATE_SLOT..MAX_STATE_SLOT) { "Invalid save-state slot: $slot" }
        executor.execute {
            val path = loadedRom?.statePathFor(slot) ?: return@execute
            val message = if (File(path).isFile && core.loadStateFile(path, settings.loadStateFlags())) {
                EmulationMessage.SAVE_STATE_LOADED
            } else {
                EmulationMessage.NO_SAVE_STATE
            }
            listener?.onMessage(message)
        }
    }

    fun cloneBatterySave(): ByteArray? = executor.submit<ByteArray?> {
        if (game == null) null else core.cloneSaveData()
    }.get(2, TimeUnit.SECONDS)

    fun restoreBatterySave(data: ByteArray): Boolean = executor.submit<Boolean> {
        game != null && core.restoreSaveData(data)
    }.get(2, TimeUnit.SECONDS)

    fun loadImportedQuickState(): Boolean = executor.submit<Boolean> {
        val path = loadedRom?.statePath ?: return@submit false
        File(path).isFile && core.loadStateFile(path, settings.loadStateFlags())
    }.get(2, TimeUnit.SECONDS)
    ...
    companion object {
        const val QUICK_STATE_SLOT = 0
        const val MAX_STATE_SLOT = 9
