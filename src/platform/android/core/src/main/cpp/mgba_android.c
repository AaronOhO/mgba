...
JNIEXPORT jbyteArray JNICALL
Java_io_mgba_android_core_MgbaNativeCore_nativeCloneSaveData(
		JNIEnv* env,
		jobject self,
		jlong handle) {
	(void) self;
	struct AndroidCore* androidCore = _fromHandle(handle);
	if (!androidCore || !androidCore->core->savedataClone) {
		return NULL;
	}
	void* data = NULL;
	size_t size = androidCore->core->savedataClone(androidCore->core, &data);
	if (!size || !data || size > INT32_MAX) {
		free(data);
		return NULL;
	}
	jbyteArray result = (*env)->NewByteArray(env, (jsize) size);
...
JNIEXPORT jboolean JNICALL
Java_io_mgba_android_core_MgbaNativeCore_nativeRestoreSaveData(
		JNIEnv* env,
		jobject self,
		jlong handle,
		jbyteArray input) {
	(void) self;
	struct AndroidCore* androidCore = _fromHandle(handle);
	if (!androidCore || !androidCore->core->savedataRestore || !input) {
		return false;
	}
	...
	bool restored = androidCore->core->savedataRestore(
		androidCore->core,
		data,
		(size_t) size,
		true);
