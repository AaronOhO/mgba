...
class EmulatorSettingsTest {
    ...
    fun `rebinding an occupied physical key swaps the two bindings`() {
        ...
        val rebound = original.withInputBinding(EmulatorKey.B, KeyEvent.KEYCODE_BUTTON_A)

        assertEquals(KeyEvent.KEYCODE_BUTTON_A, rebound.inputBindings.getValue(EmulatorKey.B))
        assertEquals(KeyEvent.KEYCODE_BUTTON_B, rebound.inputBindings.getValue(EmulatorKey.A))
    }
